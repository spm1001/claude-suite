...
| Wrong prefix issues in repo | Filter JSONL: `jq -c 'select(.id | startswith("correct-prefix-"))' issues.jsonl > clean.jsonl` |
| Database corrupted | Delete `.beads/beads.db`, reimport: `bd import -i .beads/issues.jsonl` |

---

## Database Recovery

If a repo's database is corrupted:
...
rm -f .beads/beads.db .beads/beads.db-*

# 3. Reimport from JSONL
bd import -i .beads/issues.jsonl

# 4. Verify
bd list -n 0 --json | jq 'length'
```

If the JSONL itself is corrupted (wrong prefixes), restore from git:
