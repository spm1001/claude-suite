...
Board swept nearly dry 2026-08-17 (12 items closed across suites 1.66.1–1.66.5: ardoise version-pick + print-mode `--cwd`; hublot ghost-text guard, keys settle, key-verb/menus doc; scan.py de-vacuoused at both the discovery and config layers, with regression tests in `tests/test_scan.py` and the estate config at `~/.claude/sharing-scan.json`; skill-forge checklist +4 evidence-cited rows and register-principles P9; cowork-cloud.md landed). Remaining: **deglacer accuracy** (trousse-guriku/jejozo) held behind **trousse-wazula** — deglacer is leaving trousse, waiting on bds-numeri (the deglacer plugin shipping); hold deglacer edits until the move lands. Standalone: trousse-vosujo, the `(user)`-tag conflict (lint_skill.py demands it, Cowork reportedly drops skills carrying it). Eval v2 shapes deferred on the banc board (banc-bizeko). The deja upstream issue stays parked **by decision** — don't post it, don't re-litigate.
